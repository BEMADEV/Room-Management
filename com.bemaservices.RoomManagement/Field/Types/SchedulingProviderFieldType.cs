...
{
    /// <summary>
    /// Field Type to select a single (or null) SchedulingProvider
    /// Stored as SchedulingProvider's Guid
    /// </summary>

    [FieldTypeGuid( "8F3B6F1E-9C2D-4E7A-B8C3-1A4F2E5D6C7B" )]
    [BooleanField( "Include Inactive",
        Description = "When set, inactive scheduling providers will be included in the list.",
    ...
    public class SchedulingProviderFieldType : UniversalItemPickerFieldType
    {
        ...
        {
            var allSchedulingProviders = new SchedulingProviderService( new RockContext() ).Queryable().AsNoTracking();

            if ( configurationValues == null )
            {
                return allSchedulingProviders.ToDictionary( c => c.Guid.ToString(), c => c.Name );
            }
            ...
            var schedulingProviderList = allSchedulingProviders
                .Where( c => ( c.IsActive || includeInactive ) )
                .ToList();

            return schedulingProviderList.ToDictionary( c => c.Guid.ToString(), c => c.Name );
        }          
    }
}
