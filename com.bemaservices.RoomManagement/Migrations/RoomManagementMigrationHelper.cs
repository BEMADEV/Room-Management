...
namespace com.bemaservices.RoomManagement.Migrations
{
    ...
    public class RoomManagementMigrationHelper
    {
        ...
        {
            // Only remove duplicates if there are multiple entity types with the same name
        ...
        /// <summary>
        /// Updates the entity type GUID only if an entity type with the specified name exists.
        /// This is useful for migrating from old namespace (e.g., centralaz) to new namespace (e.g., bemaservices)
        /// without creating conflicts if the entity type doesn't exist yet.
        /// </summary>
        /// <param name="name">The name of the entity type.</param>
        /// <param name="guid">The GUID to set.</param>
        public void UpdateEntityTypeGuidIfExists( string name, string guid )
        {
            Migration.Sql( string.Format( @"
                -- Only update if an entity type with this name exists
...
",
        ...
        public void UpdateEntityTypeByGuid( string name, string guid, bool isEntity, bool isSecured )
        {
            ...
            Migration.Sql( string.Format( @"
                ...
                ELSE
                BEGIN
                    -- Check if there's exactly one entity type with this name
                    -- If so, update it instead of deleting and recreating
                    ...
                    BEGIN
                        ...
                        WHERE [Name] = '{0}'
                    END
                    ELSE IF EXISTS ( SELECT [Id] FROM [EntityType] WHERE [Name] = '{0}' )
                    BEGIN
                        -- Multiple exist, update one matching the name
                        UPDATE [EntityType] SET
                            [IsEntity] = {1},
                            [IsSecured] = {2},
                            [Guid] = '{3}'
		...
		public void UpdateEntityTypeByGuid( string name, string friendlyName, string assemblyName, bool isEntity, bool isSecured, string guid )
		{
			...
			Migration.Sql( string.Format( @"
				...
				BEGIN
					...
					SET @EntityCount = (SELECT COUNT(*) FROM [EntityType] WHERE [Name] = '{0}')
					SET @Guid = (SELECT TOP 1 [Guid] FROM [EntityType] WHERE [Name] = '{0}')
					...
					ELSE
					BEGIN
						-- If there's exactly one entity type with this name, update it
						-- Otherwise update but there may be duplicates to clean up
						UPDATE [EntityType] SET
							[FriendlyName] = '{1}',
							[AssemblyName] = '{2}',
							[IsEntity] = {3},
							...
							[Guid] = '{5}'
						WHERE [Name] = '{0}'
						AND (@EntityCount = 1 OR [Guid] = @Guid)
